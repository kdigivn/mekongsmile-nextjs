...
import UserBookings from "./page-content";
...
export async function generateMetadata(): Promise<Metadata> {
  const page: WordpressPage | null = await getEnvPage(
    PageGroupEnvKeyEnum.USER_BOOKINGS
  );
  ...
  if (page) {
    if (page?.seo) {
      ...
      return {
        ...metaData,
        alternates: {
          canonical: baseUrl + "/user/bookings/",
        },
        openGraph: {
          ...metaData.openGraph,
  ...
  const { t } = await getServerTranslation("vi", "user/bookings");
  return {
    title: t("title"),
  };
}
...
export default async function Page({ params, searchParams }: Props) {
  ...
  return (
    <>
      ...
      />
      <UserBookings params={params} searchParams={searchParams} />
    </>
  );
}
