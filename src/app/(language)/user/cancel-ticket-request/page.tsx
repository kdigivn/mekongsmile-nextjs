...
import UserCancelTicketRequest from "./page-content";
...
export async function generateMetadata(): Promise<Metadata> {
  const page: WordpressPage | null = await getEnvPage(
    PageGroupEnvKeyEnum.USER_CANCEL_TICKET_REQUEST
  );
  ...
  if (page) {
    if (page?.seo) {
      ...
      return {
        ...metaData,
        alternates: {
          canonical: baseUrl + "/user/cancel-ticket-request/",
        },
        openGraph: {
          ...metaData.openGraph,
          url: baseUrl + "/user/cancel-ticket-request/",
        },
      } as Metadata;
    }
  ...
  const { t } = await getServerTranslation("vi", "user/cancel-ticket-request");
...
export default async function Page({ params, searchParams }: Props) {
  ...
  return (
    <>
      ...
      />
      <UserCancelTicketRequest params={params} searchParams={searchParams} />
    </>
  );
}
