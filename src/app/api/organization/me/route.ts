...
import { Organization } from "@/services/apis/organizations/types/organization";
...
export async function GET(request: NextRequest) {
  ...
  const requestUrl = `${buildApiPath(
    FerryTicketApiEndpoints.v1.organization.me
  )}${request.nextUrl.search}`;

  const { data, status } = await fetch(requestUrl, {
    method: "GET",
    headers: header,
  }).then(wrapperFetchJsonResponse<Organization>);
