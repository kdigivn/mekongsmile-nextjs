...
import { VoyagesResponse } from "@/services/apis/voyages/voyages.service";
...
export async function GET(request: NextRequest) {
  ...
  const requestUrl = `${buildApiPath(
    FerryTicketApiEndpoints.v1.voyages.findAllRelateByLocationAndDateRange
  )}${request.nextUrl.search}`;

  const { data, status } = await fetch(requestUrl, {
    method: "GET",
    headers: header,
  }).then(wrapperFetchJsonResponse<VoyagesResponse>);
