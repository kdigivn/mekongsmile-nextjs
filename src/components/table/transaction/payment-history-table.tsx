...
import { useGetTransactionListQueryCursorWithoutFilter } from "@/services/apis/bookings/transactions/transactions.service";
...
function PaymentHistoryTable({ columns }: DataTableProps) {
  ...
  const {
    ...
    transactionsLoading,
  } = useGetTransactionListQueryCursorWithoutFilter(filterTransactionRequest);
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell
                        key={cell.id}
                        className={cn(
                          "min-w-52 overflow-hidden text-ellipsis border align-top",
                          cell.column.id === "id" && "min-w-40 max-w-52"
                        )}
                      >
                        {flexRender(
